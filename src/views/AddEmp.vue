...
import BTModalfailUpload from "../components/BTModalfailUpload.vue";
import { ref } from "vue";
      ...
      if (event && event.target && event.target.files.length > 0) {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          reader.onload = () => {
            ...
            const imageExists = this.uploadedImages.some(
    ...
    async uploadImages() {
      try {
        ...
        for (let i = 0; i < this.uploadedImages.length; i++) {
          ...
          const blob = await fetch(this.uploadedImages[i].url).then((res) =>
            res.blob(),
          );
        ...
        if(!this.errcam_2 && !this.errcam_1){
          this.modalOpen = true;
        }
      } catch (error) {
        console.error("Error uploading images:", error);
      } finally {
        this.loading = false; // Set loading state to false after image upload completes
      }
    },
    deleteall() {
...
<template>
  <div class="flex h-screen bg-[#F5F8FF] font-Kanit">
    ...
    <!-- main container -->
    <div
      class="w-full bg-white rounded-[14px] mx-[20px] my-[40px] flex-[0.9] p-[30px] shadow-md overflow-y-auto" style="box-shadow: 0px 0px 10px rgba(245, 248, 255, 1);"
    >
      <!-- section_1 -->
      ...
      <div class="flex h-[85%]">
        ...
        <div class="flex flex-col w-[60%]">
          ...
          <div class="h-[20%] flex items-center justify-center">
            <div class="mr-[15px]">
              <BTButton :icon="Webcam" title="เว็บแคม" :onClick="startCamera" />
            </div>
            <div class="mr-[15px]">
              <BTButton :icon="Upload" title="อัปโหลด" :onUpload="AddImage" />
            </div>
            <div v-if="this.uploadedImages.length > 0">
              <BTButton title="ลบทั้งหมด" :onClick="deleteall" />
            </div>
          </div>
        </div>
        <!-- information -->
        <div class="w-[40%] justify-center items-center flex">
          <div class="flex flex-col">
            ...
            <div
              v-if="opC"
              class="relative w-[12rem] h-[12rem] rounded-[14px] border-[#000] border-[1px] mb-[50px]"
            >
              <div
                @click="clickadd"
                class="flex items-center justify-center absolute w-[50px] h-[50px] hover:w-[55px] hover:h-[55px] bg-[#F5F8FF] shadow-md cursor-pointer rounded-[5px] right-[-60px] top-1/2 transform -translate-y-1/2"
              >
                <svg
                  width="20"
                  height="19"
            ...
            <!-- upload_button -->
            <div
              class="flex items-center justify-center p-[10px] rounded-[14px] bg-[#1C1C1C] mt-[10px] hover:p-[12px] cursor-pointer"
              @click="!isDisabled && uploadImages()"
              :class="{ 'cursor-not-allowed opacity-50': isDisabled }"
            >
              <p class="text-[14px] text-[#FFF]">อัปโหลดพนักงาน</p>
            </div>
            <!-- Dialog confirmation -->
            <dialog id="my_modal_1" className="modal" :open="modalOpen">
              <div className="modal-box">
                <h3 className="font-bold text-lg">สำเร็จ</h3>
                <p className="py-4">อัปโหลดพนักงานสำเร็จแล้ว</p>
                <div className="modal-action">
                  <form method="dialog">
                    <button className="btn" @click = "refreshPage">ปิด</button>
                  </form>
                </div>
              </div>
            </dialog>
            <!-- Dialog fail point -->
            <dialog id="my_modal_1" className="modal" :open="this.errcam_1 || this.errcam_2">
              <div className="modal-box">
                <h3 className="font-bold text-lg">ไม่สามารถอัปโหลดพนักงานได้</h3>
                <p className="py-4" v-if="this.errcam_1">เกิดความขัดข้องในกล้องจุดที่ 1</p>
                <p className="" v-if="this.errcam_2">เกิดความขัดข้องในกล้องจุดที่ 2</p>
                <div className="modal-action">
                  <form method="dialog">
                    <button className="btn" @click = "errcam">ปิด</button>
                  </form>
                </div>
              </div>
            </dialog>
            <!-- loading -->
            <dialog id="my_modal_1" className="modal" :open="loading">
              <div className="modal-box">
                <span className="loading loading-spinner loading-lg m-[50px] w-14"></span>
                <p>กำลังโหลด</p>
              </div>
            </dialog>
          </div>
